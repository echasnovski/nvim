...
T['Autocommands']['resets on color scheme change'] = function()
  ...
  child.lua([[require('mini-dev.hipatterns').setup({
...
local enable = function(...) child.lua([[require('mini-dev.hipatterns').enable(...)]], { ... }) end

local get_enabled_buffers =
  function(...) return child.lua_get([[require('mini-dev.hipatterns').get_enabled_buffers(...)]], { ... }) end
...
T['enable()']['reacts to filetype change'] = function()
  ...
  set_lines({ 'xxabcdxx' })
  child.lua([[require('mini-dev.hipatterns').enable(0, _G.hipatterns_config)]])
  child.expect_screenshot()
...
T['Highlighters']['allows submatch in `pattern`'] = function()
  ...
  local validate = function(pattern)
    child.lua([[require('mini-dev.hipatterns').disable()]])
    local config = {
...
T['Highlighters']['allows callable `pattern`'] = function()
  ...
  child.lua([[require('mini-dev.hipatterns').enable(
    0,
    {
...
T['Highlighters']['respects `priority`'] = function()
  ...
  child.lua([[require('mini-dev.hipatterns').disable(0)]])
...
local disable = function(...) child.lua([[require('mini-dev.hipatterns').disable(...)]], { ... }) end
...
local toggle = function(...) child.lua([[require('mini-dev.hipatterns').toggle(...)]], { ... }) end
...
T['toggle()']['works'] = function()
  ...
  child.lua([[require('mini-dev.hipatterns').toggle(nil, test_config)]])
  child.expect_screenshot()
  eq(get_enabled_buffers(), { cur_buf_id })
...
local update = function(...) child.lua([[require('mini-dev.hipatterns').update(...)]], { ... }) end
...
T['update()']['works'] = function()
  ...
  child.lua([[require('mini-dev.hipatterns').update(0, nil, 3)]])
  child.expect_screenshot()

  -- `to_line` should be inferred as last line
  child.b.not_highlight = true
  child.lua([[require('mini-dev.hipatterns').update(0, 2, nil)]])
  child.expect_screenshot()
...
local enable_hex_color = function(...)
  child.lua(
    [[_G.hipatterns = require('mini-dev.hipatterns')
      _G.hipatterns.setup({
