...
local make_palette = function(...) return child.lua_get([[require('mini-dev.hues').make_palette(...)]], { ... }) end
...
T['get_random_base_colors()'] = new_set()

T['get_random_base_colors()']['works'] = function()
  -- With dark background
  child.o.background = 'dark'
  child.lua('math.randomseed(20230504)')
  eq(
    child.lua_get([[require('mini-dev.hues').get_random_base_colors()]]),
...
T['get_random_base_colors()']['respects `opts.get_hue`'] = function()
  for _ = 1, 2 do
    eq(
      child.lua_get([[require('mini-dev.hues').get_random_base_colors({ get_hue = function() return 0 end })]]),
      { background = '#2f1c22', foreground = '#cdc4c6' }
    )
  end

  eq(
    child.lua_get([[require('mini-dev.hues').get_random_base_colors({ get_hue = function() return 720 end })]]),
    { background = '#2f1c22', foreground = '#cdc4c6' }
  )
end

T['get_random_base_colors()']['validates arguments'] = function()
  expect.error(
    function() child.lua([[require('mini-dev.hues').get_random_base_colors({ get_hue = 1 })]]) end,
    '`get_hue`.*callable'
  )
end
